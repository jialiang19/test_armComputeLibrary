...
#include "caffe_weights.h" using namespace arm_compute;
using namespace std;
...
int main(int argc, const char **argv)
{
	...
	*/ 
	...
	/* this is the set up for input as one image, 
	   ...
	   runing out the memory. I did not have further investigations */  
	...
	*/  
	...
	cout << "Median time is " << (timeVec[50]+timeVec[51])/2 << endl;  
...
//	cout << "time passed is " << elapsed_time << endl; 	
...
// The following is just used for testing the specific value at each layer //
	...
	while(1) {
		locationVec.clear(); 
		cout << "Dimension type location" << endl; 
        	getline(cin, line); 
		iss.clear(); 
		iss.str(line.c_str()); 	
		iss >> NumofD >> IString; 
	...
	if(NumofD == 3){
		layer_index = layername_map[IString]; 	  
		switch(layer_index) {
			case 0: 
		...
		weight_index = layername_map[IString]; 	  
		switch(weight_index) {
			case 0: 
	...
	/* value testing  
	vector <int> location; 
	location.push_back(77);
	location.push_back(5);
	location.push_back(2); 
	cout << endl << "value after normalization layer at position (0,0,0) " << endl; 
	JPrintTensor4D(MyAlexNet.w[2].get(), location);  
	*/ 
